    ...
    if file_extension != 'Cancelar':
        ...
        if confirmation == 'Yes':            
            ...
            try:            
                ...
                sg.PopupOK('Ocurrió un error al querer guardar el meme...\nPor favor, verifique que el archivo "config.json" se encuentre en la carpeta "data" y que las rutas en el menú de Configuración sean correctas.')
...
def create_window(img_data, img_path):
    """
...
def generador_memes(user_Data, my_template):
    ...
    try:
        ...
        sg.PopupOK('No se encuentra el archivo "temples_memes.json" en la carpeta "data".')
        img_data = None
    ...
    if img_data is not None: 
        ...
        if (img_data is None) or (img_path is None):

            sg.PopupOK(f"No se encuentra alguno de los siguientes archivos en la carpeta 'data':\n  - 'config.json'\n  - 'temples_memes.json'\n Por favor, verifique que existan para continuar.")

        else:

            my_window = create_window(img_data, img_path)
