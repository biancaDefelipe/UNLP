...
def crear_config (json_path, paths):
    ...
    # modificado: agrego try except
    try:
        with open(json_path,"w") as archivo:
            json.dump(default_paths, archivo, indent=4)
    
    # except por si no existe la carpeta data
    except NotADirectoryError: 
        sg.PopupOK("La carpeta 'data' fue borrada o no existe")
    
    # except por si no existe el archivo config.json
    except FileNotFoundError: 
        sg.PopupOK("El archivo 'config.json' fue borrado o no existe")
