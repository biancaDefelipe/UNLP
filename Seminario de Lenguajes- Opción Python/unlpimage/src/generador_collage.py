...
def get_images_and_buttons(dicci_diseño, diseño_elegido):
    ...
    try:
        ...
        sg.popup("El archivo 'config.json' fue borrado o no existe. Por favor, vuelva a iniciar la aplicación para generarlo.")
...
def save_collage(collages_path, imagen_original_con_titulo):
    ...
    if name and not name.isspace():
        ...
        if file_extension and file_extension != 'Cancelar':
            name = name + '.' + file_extension
            ruta_collage = os.path.join(collages_path, name)
            if file_extension == 'jpg':
                jpg_image = imagen_original_con_titulo.convert('RGB')
                jpg_image.save(ruta_collage)   
            else:
        ...
        sg.popup("Por favor ingrese un titulo para el collage")
...
def generador_collage(dicci_user, img):
    ...
    if data_json is None: 
        sg.PopupOK("El archivo 'diseños_collage.json' fue borrado o no existe")
    else: 
        ...
        if collages_path is None:
            sg.PopupOK("El archivo 'config.json' fue borrado o no existe. Por favor, vuelva a iniciar la aplicación para generarlo.")
